...
def mha(qkv_HDm3Dh: Array, residual_SDm: Array):
    ...
    # out_SDm = jnp.einsum(out_HSDh, 'hsd->s(dh)')

    return out_SDm


def make_mha_weights(d_model, d_head, n_heads, key):
...
def make_ff_weights(d_model, mlp_ratio, key):
...
def make_block_weights(d_model, d_head, n_heads, mlp_ratio, key):
    ln1_params = make_layer_norm_params(d_model)
    mha_weights = make_mha_weights(d_model, d_head, n_heads, key)
    ln2_params = make_layer_norm_params(d_model)
    ff_weights = make_ff_weights(d_model, mlp_ratio, key)
    return ln1_params, mha_weights, ln2_params, ff_weights
...
def make_blocks_weights(d_model, d_head, n_heads, mlp_ratio, n_layers, key):
    return [
        make_block_weights(d_model, d_head, n_heads, mlp_ratio, key) for _ in range(n_layers)
    ]
...
def make_model_weights(cfg: ModelCfg, key):
    w_VDm = make_embed_weights(cfg.d_vocab, cfg.d_model, key)
    blocks_params = make_blocks_weights(
        cfg.d_model, cfg.d_head, cfg.n_heads, cfg.mlp_ratio, cfg.n_layers, key
    )
    final_layer_norm_params = make_layer_norm_params(cfg.d_model)
    w_DmV = make_unembed_weights(cfg.d_model, cfg.d_vocab, key)
    return w_VDm, blocks_params, final_layer_norm_params, w_DmV

if __name__ == "__main__":
    key = jax.random.PRNGKey(0)
    key, subkey = jax.random.split(key)
    params = make_model_weights(
        ModelCfg(
            d_vocab=16,
            d_model=128,
            n_heads=4,
